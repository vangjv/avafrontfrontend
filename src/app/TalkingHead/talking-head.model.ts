...
class TalkingHead {
  ...
  posePropNames: any[];
  poseName: string;
  poseWeightOnLeft: any;
  poseCurrentTemplate: any;
  poseBase: any;
  poseTarget:any;
  poseAvatar: null;
  avatarHeight: number;
  ...
  lightDirect: THREE.DirectionalLight;
  lightSpot: THREE.SpotLight;
  ...
  */
  propsToThreeObjects(p:any) {
    const r = {};
    for( let [key,val] of Object.entries(p) ) {
      const ids = key.split('.');
      let v;
      if ( ids[1] === 'position' || ids[1] === 'scale' ) {
        v = new THREE.Vector3((val as any).x,(val as any).y,(val as any).z);
      } else if ( ids[1] === 'rotation' ) {
        key = ids[0] + '.quaternion';
        v = new THREE.Quaternion().setFromEuler(new THREE.Euler((val as any).x,(val as any).y,(val as any).z,'XYZ')).normalize();
      } else if ( ids[1] === 'quaternion' ) {
        v = new THREE.Quaternion((val as any).x,(val as any).y,(val as any).z,(val as any).w).normalize();
      }
      if (v) {
        r[key as string] = v;
      }
  ...
  * @return {Object} A new pose object.
  */
  poseFactory(template:any, duration?:any) {
    const o:any = { template: template, props: {} };
    Object.entries(template.props).forEach( (x:any) => {
  ...
  */
  setPoseFromTemplate(template:any) {
    ...
    Object.keys(this.poseDelta.props).forEach( (key:any) => {
  ...
  */
  getBaselineValue( mt:any ) {
    if ( mt === 'eyesRotateY' ) {
      const ll:any = this.getBaselineValue('eyeLookOutLeft');
      if ( ll === undefined ) return undefined;
      const lr:any = this.getBaselineValue('eyeLookInLeft');
      ...
      const d:any = this.getBaselineValue('eyesLookDown');
      if ( d === undefined ) return undefined;
      const u:any = this.getBaselineValue('eyeLookUp');
  ...
  */
  getFixedValue( mt:any ) {
    if ( mt === 'eyesRotateY' ) {
      const ll:any = this.getFixedValue('eyeLookOutLeft');
      if ( ll === undefined ) return undefined;
      const lr:any = this.getFixedValue('eyeLookInLeft');
      if ( lr === undefined ) return undefined;
      ...
      const d:any = this.getFixedValue('eyesLookDown');
      if ( d === undefined ) return undefined;
      const u:any = this.getFixedValue('eyeLookUp');
      if ( u === undefined ) return undefined;
      return d - u;
    } else {
  ...
  */
  animFactory( t:any, loop:any = false, scaleTime = 1, scaleValue = 1 ) {
  ...
  * @param {number} [scale=0.01] Position scale factor
  */
  async playPose(url:any, onprogress:any=null, dur=5, ndx=0, scale=0.01) {
  ...
  */
  ikSolve(ik:any, target:any=null, relative=false, d=null) {

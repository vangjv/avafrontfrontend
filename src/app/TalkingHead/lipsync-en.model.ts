...
class LipsyncEn {
  ...
  */
  constructor() {
    ...
    Object.keys(this.rules).forEach( key => {
      this.rules[key] = this.rules[key].map( (rule:any) => {
        ...
        const o:any = { regex: '', move: 0, visemes: [] };
        ...
        if ( strVisemes.length ) {
          strVisemes.split(' ').forEach( (viseme:any) => {
  ...
  convert_digit_by_digit(num:any) {
  ...
  */
  preProcessText(s:any) {
    return s.replace('/[#_*\'\":;]/g','')
      .replace( this.symbolsReg, (symbol:any) => {
  ...
  */
  wordsToVisemes(w:any) {
    let o:any = { words: w.toUpperCase(), visemes: [], times: [], durations: [], i:0 };
    let t = 0;
    ...
    while( o.i < chars.length ) {
      ...
      if ( ruleset ) {
        for(let i=0; i<ruleset.length; i++) {
          ...
          if ( matches ) {
            rule.visemes.forEach( (viseme:any) => {
